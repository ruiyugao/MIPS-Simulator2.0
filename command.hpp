...
static int reg[32], heapLength = 0, conti = 0;
...
static map<string, INSTRUCT> insType;
static cmd COMMAND;
static REG Reg;
static MEM Mem;
...
/*!
 * Here to put the register and the instructions I have implemented.
 */
void init()
{
...
/*!
 * Take each instruction into struct command with type and size.
 * @param s
 * @return
 */
...
/*!
 * Here is to deal with data section.
 * I do not maintain it well cause I still haven't understand how to do it...
 * @param CMD
 */
inline void dataExcute(command &CMD)
{
    switch (CMD.type) {
        case Align: {
            ...
            n          = pow(2, n);
            heapLength = ((heapLength - 1) / n + 1) * n;
            break;
        }
        case ascii: {
            string   str = CMD.v[0];
            int      len = str.size();
            for (int i   = 0; i < len; ++i) Memory[heapLength++] = str[i];
            break;
        }
        case asciiz: {
            string   str         = CMD.v[0];
...
/*!
 * Here is to fetch the instr from the memory
 */
inline void IF()
{
    ++conti;
...
/*!
 * Here is to deal with instructions without overflow
 */
inline void IDDP()
{
    if (!line.empty()) {
    ...
    if (!memo.empty()) {
        ...
        switch (Mem.Itype) {
            ...
            case lw: {
                Reg.Itype  = Mem.Itype;
                Reg.Rtype  = Mem.Rtype;
                Reg.result = Memory[Mem.pos] | Memory[Mem.pos + 1] << 8 | Memory[Mem.pos + 2] << 16 |
                             Memory[Mem.pos + 3] << 24;
                break;
            }
            case sb: {
